...
class TD3(object):
    def __init__(self, cfg):
        ...
        self.lstm = nn.LSTM(self.n_states + self.n_actions, 128, 2, batch_first=True)  # 处理历史轨迹(input_size, hidden_size, num_layers)
        ...
        self.actor = Actor(cfg.n_states, cfg.n_actions, self.max_action, self.lstm, self.la).to(self.device)
        self.actor_target = copy.deepcopy(self.actor)  # 将一个网络赋值给另一个网络，且不相互影响
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=1e-4)

        self.critic = Critic(cfg.n_states, cfg.n_actions, self.lstm, self.la).to(self.device)
